 ...
 * typecheck, lint, test and build without a word, so the rule in CLAUDE.md held
...
for (const [file, expected] of Object.entries(EXPECTED)) {
  test(`${file} declares no dependency the architecture gate has not seen`, () => {
    ...
    const why = `\n\n${file} changed its dependencies. That is an architecture gate — see "No new\ndependency without asking" in CLAUDE.md. If the change is deliberate, update\nEXPECTED in scripts/deps.test.mts in the same commit.\n`;
...
test("the rejected alternatives are not installed anywhere", () => {
  // The stack list in CLAUDE.md names these by name. Catching them here rather
  ...
  for (const file of Object.keys(EXPECTED)) {
    ...
    for (const banned of rejected) {
      assert.ok(
        ...
        `${file} installs ${banned}, which the stack rules in CLAUDE.md rule out.`,
      );
    }
  }
});
