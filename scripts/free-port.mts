#!/usr/bin/env node
/**
 * Frees a TCP port before a dev server binds it. `node scripts/free-port.mts 8787`
 *
 * **Why this exists.** `pnpm dev:ytmusic` runs uvicorn with
...
function run(file: string, args: string[]): string {
  try {
    ...
    // `lsof` and `findstr` exit non-zero for "no match", which is the common case. The
    // error execFileSync throws carries whatever the command printed before it exited.
