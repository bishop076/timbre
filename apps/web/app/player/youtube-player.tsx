 ...
 * invisible in local development and why the advice to log the numeric code found
 ...
 * stops being true of within a second or two.
 */
 ...
 * answered 403 to the media — the silent stall — every time, UNSTARTED and BUFFERING for
 ...
 * the same browser, so the address was not simply barred.
 *
  ...
  const watchForStall = useCallback(
    (id: string) => {
      ...
      stallTimer.current = setTimeout(() => {
        ...
        // Logged like an error code would be: the state is the only number there is.
      ...
      playerRef.current = new YT.Player(host, {
        ...
        events: {
          ...
          onError: (event: { data: number }) => {
            ...
            // Log the raw code, never just the sentence: a player under
            // A player below 200×200 and a barred embed produce the same friendly text, so
            ...
            // ladder leave YouTube once a second upload says the same.
