...
interface PlayerState {
  ...
  /** Whether the now-playing panel is shown. Hiding only clips the player: the IFrame API stops playback below 200×200. */
  panelOpen: boolean;
  ...
  volume: number;
  muted: boolean;
  /** What to play after the queue (`recommend.ts`). Fetched on every track change so continuing costs no round trip. */
  radio: Song[];
  shuffle: boolean;
  repeat: RepeatMode;
 ...
 * and 246 available countries. An ad blocker produces the same sentence again. This project is on
...
export function PlayerProvider({ children }: { children: ReactNode }) {
   ...
   * `goTo(queue.length)`, so the closure's copy is one item short.
   *
   ...
   * could not reach. That is the fall-through failure whose named remedy
   * this is.
   *
   ...
   * and the bug where a skip and a track ending drifted apart.
   *
  ...
  const load = useCallback(
    async (song: Song, prefer?: string) => {
      ...
      if (direct) {
        ...
        // Warm the fall-through list while the first copy is still loading.
        // Resolving on failure instead cost fail → round trip → retry, which a
  ...
  const handleError = useCallback(
    async (
      ...
      try {
        ...
        // stalled identically. Walking five candidates at `STALL_MS` each would be
        ...
        // 2026-09-10, every video 150 from one VPN exit, over YouTube's bot wall.
      ...
      // the VPN fall-through failure this rescue exists for.
      //
       ...
       * failure this file has already paid for once in `advance`.
       *
