...
//
// B-37, B-38 and B-46 were all live in production while `pnpm typecheck`, `pnpm lint`, the
...
// what this script automates. See S-11 in docs/SUGGESTIONS.md for the argument.
//
// It drives Chrome over the DevTools Protocol with nothing but Node builtins. Playwright would
// be shorter and is the obvious tool, but "no new dependency without asking" in CLAUDE.md is an
...
//
// The first half is B-38: a CSP violation is reported on an event and nowhere else, and the
...
async function visit(chrome, route) {
  ...
  tab.on("Network.responseReceived", ({ type, response }) => {
    ...
    // `/api/art` answering 403 is B-37 exactly: the allowlist refused a cover this app itself
...
// B-46's second half: an unsupported link is "that link isn't from a service Timbre can play"
