...
- **Every route goes through `lib/api.ts`** — `queryRoute(schema, message, handler)` when there are
  query params, `guard(request, budget)` otherwise. A route that rate-limits itself by hand or parses
  its own search params is a route that got it wrong.
