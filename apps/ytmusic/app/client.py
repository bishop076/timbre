...
#
# That bound is per *call*, and S-24 read it as if it were per request. It is not: no route
...
# each that is 24s and 56s of held thread, which is worse than the 30s S-24 removed. So the
...
# 619 MB peak, measured against a socket that simply never stopped. This is S-29's finding in
...
# not one goes over. So S-29's 4 MB would cut real playlists in half; twice the largest thing
# YouTube has ever been seen to send leaves that ceiling clear without inviting a heap back.
#
# Per call, not per request, unlike the deadline. S-32's lesson is that a bound on one call is
...
class UpstreamTooLarge(requests.exceptions.RequestException):
    ...
    A `RequestException` because every route already turns one into the flat 502 that S-14
...
def _read_capped(response: requests.Response, due: float, limit: int, budget: float) -> None:
    ...
    budget S-32 introduced, so a trickle costs what a hang costs and no more.
