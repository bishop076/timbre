...
— which is the reason it is not done.
...
treat the sidecar as blocked; nothing downstream will help.
...
donation link puts a Hobby project in breach. Both matter, and so does everything
else that changes the moment this becomes reachable from the open internet.
**Work all of that out before the first deploy**, not after.
