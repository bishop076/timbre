...
| `SOUNDCLOUD_API_BASE` | an `api-v2`-shaped base *you* run. Unset by default; setting it moves the technique and the terms exposure onto you. |
...
correct and expected — its catalogue search is gated.
...
art track leading instead means the ranking regressed.
...
to an ad blocker, which is what made it cost a day the first time.
...
— the 403 stall. That was the embed **host**, not the network — youtube.com itself
played in the next tab, and the identical player served from `youtube-nocookie.com` plays.
That is the fix. If a deployed build shows the 0:00 spinner again, YouTube's judgement of
